...
- `verified`: the linked probe was run by the user or through explicitly
  authorized `agent:dc_mcp` execution in the recorded NX version, and all
...
schema v1 manual evidence or schema v2 `user:nx_ui`/authorized `agent:dc_mcp`
evidence with applicable artifact checks. Agents must never launch or close NX;
MCP execution must follow `references/mcp-runtime.md`.
